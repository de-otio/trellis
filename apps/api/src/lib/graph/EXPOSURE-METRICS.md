...
- Per-viewer logs would expose a fine-grained social graph (who saw what, when)
  ...
  with the data-minimization commitment (internal design note:
  `enshittification-resistance/04-data-minimization`).
- The aggregate counters carry sufficient statistical power to detect
  concentration effects at the population level (internal design note:
  `algorithmic-norm-misperception/02-prelaunch-actions §3`).
