...
The reference client is Flutter; the flow is: call `POST /user/export`,
poll `GET /user/export/status/:jobId` until `completed`, then fetch
...
- **Total Time**: up to 24 hours (acceptable per requirements)
...
- Consider pagination for very large exports (future)
- Consider compression for large files (future)
...
For current testing patterns see [doc/02-technical/development/testing.md](../development/testing.md).
...
- Initial API design
- Support for JSON and ActivityPub formats
...
- [ActivityPub](../architecture/07-activitypub.md)
