...
on the long-term roadmap (data localisation for additional regulatory
regions such as CN; the identity-federation design's compliance principles
include cross-border placement), and the `Tenant` model already carries a
`region` field.
