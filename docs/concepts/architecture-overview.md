...
How the pieces relate, top to bottom: vertical applications reach the core
only through the extension API; the core reaches all infrastructure through
provider-neutral ports; per-port adapters let the same codebase run on a
single node or on managed cloud services.

```mermaid
flowchart TB
    subgraph verticals["Vertical applications — built on the core by developers"]
        V["a vertical serves many tenant communities<br/>(see Tenancy Model)"]
    end

    EXT["Extension API — @de-otio/trellis-extension-api (MIT), the contractual seam"]

    subgraph core["Trellis core — @de-otio/trellis (AGPL-3.0)"]
        CAP["accounts & identity · communities & feeds ·<br/>media with fail-closed moderation · background processing ·<br/>data export · ActivityPub federation (off by default, gated)"]
    end

    subgraph ports["Provider-neutral ports — @de-otio/saas-foundation (Apache-2.0)"]
        direction LR
        KV["key-value"]
        OS["object storage"]
        Q["queue"]
    ...
    subgraph adapters["Adapters — each behind its port's contract"]
        direction LR
        A1["Postgres ·<br/>DynamoDB ·<br/>in-memory"]
        A2["any<br/>S3-compatible<br/>store"]
        A3["SQS-<br/>compatible<br/>queues"]
        A4["SMTP ·<br/>provider<br/>APIs"]
        A5["Keycloak<br/>(self-hosted) ·<br/>managed OIDC"]
    end

    subgraph deploy["Deployment shapes — the same codebase"]
        direction LR
        D1["single node<br/>(Docker Compose)"]
        D2["managed cloud<br/>services"]
    end
