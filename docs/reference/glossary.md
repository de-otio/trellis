...
| **Extension** | A pluggable module (`TrellisExtension`) that adds a domain-specific entity type, metadata schema, routes, recommendation strategies, and lifecycle hooks. A vertical defines it in its own repository and registers it via `registerExtension()` at startup; the core never statically imports an extension. |
